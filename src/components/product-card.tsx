...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    ...
    <div className="product-copy"><p className="brand">{product.brand}</p><h3>{product.name}</h3><p className="muted">{product.colour}</p><p className="price">{pounds(product.price)}</p>
      <div className="product-actions"><label><span className="sr-only">Size for {product.name}</span><select value={size} onChange={(event) => setSize(event.target.value)}>{product.sizes.map((item) => <option key={item}>{item}</option>)}</select></label><button type="button" onClick={addToBag} disabled={status === "adding"} aria-live="polite">{status === "adding" ? "Adding…" : status === "added" ? "Added ✓" : status === "error" ? "Try again" : "Add to bag"}</button></div>
    </div>
  </article>;
}
