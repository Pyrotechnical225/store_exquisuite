...
export default function Home() {
  ...
  const designer = products.filter((product) => product.category === "Designer");
  ...
  return <>
    ...
    <section className="section shell" id="women"><div className="section-heading"><div><p className="eyebrow dark">Modern womenswear</p><h2>Women’s collection</h2></div><p>Refined layers, soft tailoring and occasion-ready silhouettes for a versatile wardrobe.</p></div><div className="product-grid">{women.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>
    <section className="section shell" id="men"><div className="section-heading"><div><p className="eyebrow dark">Considered menswear</p><h2>Men’s collection</h2></div><p>Relaxed tailoring and elevated essentials designed to work across the week.</p></div><div className="product-grid">{men.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>
    <section className="section shell" id="designer"><div className="section-heading"><div><p className="eyebrow dark">Curated labels</p><h2>Designer focus</h2></div><p>Distinctive accessories, tailoring and premium wardrobe foundations.</p></div><div className="product-grid">{designer.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>
